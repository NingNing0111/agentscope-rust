...
`reply_stream` 产出 33 种 `AgentEvent`，完整覆盖回复生命周期、模型调用、文本/思考增量、工具调用与结果、人工确认/中断。完整分发见 [`examples/chat`](../../../../../examples/chat/) 与 [`examples/agent`](../../../../../examples/agent/)。

## 下一步

<CardGroup cols={2}>
  <Card title="配置智能体" icon="gear" href="/versions/0.1.0/zh/building-blocks/agent/configure-agent">
    如何设置模型、工具、权限与配置。
  </Card>
  <Card title="运行智能体" icon="play" href="/versions/0.1.0/zh/building-blocks/agent/run-agent">
    如何回复、流式、观察与持久化状态。
  </Card>
  <Card title="人机交互" icon="user-check" href="/versions/0.1.0/zh/building-blocks/agent/human-in-the-loop">
    如何暂停等待用户确认或外部执行。
  </Card>
  <Card title="中断智能体" icon="hand" href="/versions/0.1.0/zh/building-blocks/agent/interrupt-agent">
    如何干净地停止运行中或暂停中的智能体。
  </Card>
</CardGroup>
