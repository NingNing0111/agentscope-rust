...
├── examples/        # 可运行示例(pi-rust coding Agent 等)
├── docs/            # 文档站点(zh / en)
...
| `agent_scope_dashscope` | Provider | DashScope(Qwen)实现 | `DashScopeChatModel`、`DashScopeEmbeddingModel`、`DashScopeFormatter` |
...
- `agent_scope_dashscope` 是当前**唯一内置 Provider**;接入新厂商需在独立 crate 实现 `ChatModel` / `EmbeddingModel` trait。
...
| 直接调用某个模型 | `agent_scope_model` + `agent_scope_dashscope` |
| 构建一个能对话 + 用工具的 Agent | `agent_scope_agent` + `agent_scope_dashscope` + `agent_scope_tool` + `agent_scope_message` |
