...
工作空间示例：创建 `LocalWorkspace`，绑定到 `ReActAgent`，让 agent 自动获得 workspace 内置工具（`Bash`/`Read`/`Write`/`Edit`/`Grep`/`Glob`/`ResetTools`/`Skill`），但使用只读 permission context，仅允许 `Read`/`Glob`/`Grep` 执行。
...
- 示例以只读权限运行：虽然 workspace 会注入 `Bash`/`Write`/`Edit` 等工具，permission context 只允许 `Read`/`Glob`/`Grep`，其他未分类工具调用会被拒绝。
