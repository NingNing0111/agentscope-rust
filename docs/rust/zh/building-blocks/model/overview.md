...
<Note>
**Rust 实现状态**: 已实现（兼容等级 L1）。当前内置模型 provider 为 DashScope（OpenAI 兼容端点）。**TTS 在 Rust 中为「计划中」**，Realtime 模型亦未实现。
</Note>
...
| trait | 职责 | 内置实现 |
|-------|------|----------|
| `ChatModel`（`agent_scope_model`） | 对话与工具调用 | `DashScopeChatModel` |
| `EmbeddingModel`（`agent_scope_embedding`） | 文本转稠密向量 | `DashScopeEmbeddingModel` |

Rust 版模型层保持简洁——模型直接接收 API Key（或环境变量）与模型名构造，无独立的 Credential 对象：

```rust
let model = DashScopeChatModel::new(&api_key, "qwen-plus").with_stream(true);
```
...
| 模型族 | Rust 状态 | 类型 |
|--------|-----------|------|
| LLM（Chat Model） | ✅ 已实现 | `ChatModel` trait、`DashScopeChatModel` |
...
let model = DashScopeChatModel::new("sk-your-key", "qwen-plus");
let stream_model = DashScopeChatModel::new("sk-your-key", "qwen-plus").with_stream(true);
```
