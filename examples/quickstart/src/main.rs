...
//!
//! Shows the four steps to run a conversational agent with a DashScope/Qwen
//! chat model: credential, model, toolkit, and a ReActAgent. Both entry
...
async fn main() -> anyhow::Result<()> {
    ...
    // Load a .env file from the repo root, if present (DASHSCOPE_API_KEY=...).
    dotenv::dotenv().ok();

    // 1. Credential — read from the environment (or a .env file in the repo root).
    let api_key = std::env::var("DASHSCOPE_API_KEY").map_err(|_| {
        anyhow::anyhow!(
            "error: 缺少环境变量 DASHSCOPE_API_KEY。\n\
             运行前请设置：\n    export DASHSCOPE_API_KEY=sk-your-key\n\
             或将 DASHSCOPE_API_KEY=sk-your-key 写入仓库根目录 .env 文件（示例会通过 dotenv 自动加载）。"
        )
    })?;

    // 2. Chat model — DashScope / Qwen via an OpenAI-compatible endpoint.
